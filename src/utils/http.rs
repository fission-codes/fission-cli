...
impl HttpRequest {
    pub fn new(addr: &str, args: &HashMap<&str, &str>, is_multipart: bool) -> Self {
        let owned_args: HashMap<String, String> = args
            .iter()
    ...
    pub fn add_body(&mut self, headers: &HashMap<&str, &str>, body: &[u8]) {
        ...
        let owned_headers: HashMap<String, String> = headers
            ...
            .map(|(key, val)| (key.to_string(), val.to_string()))
            .collect();
        self.post_options.push(PostOptions {
            headers: owned_headers,
            body: owned_body,
        });
    }
    pub fn get_url(&self) -> String {
        let mut arg_str: String = self
            .args
            .iter()
...
impl HttpHandler {
    ...
    pub async fn send_request(&mut self, options: &HttpRequest) -> Result<Vec<u8>> {
        ...
        println!(
            "{}",
            (format!("sending get or post request to \"{}\"", request_url).blue())
        );
        let mut request_builder = Request::builder().method(Method::POST).uri(request_url);
        let request = match options.is_multipart {
            false => match options.post_options.get(0) {
                Some(options) => {
        ...
        let mut response = self
            .tokio
            .block_on(async { self.http_client.request(request).await })?;
